...
# start vttablets for keyspace duo
for i in 100 101 102; do
	CELL=zone1 TABLET_UID=$i ../common/scripts/mysqlctl-up.sh
	CELL=zone1 KEYSPACE=duo TABLET_UID=$i ../common/scripts/vttablet-up.sh
done
...
wait_for_healthy_shard duo 0 || exit 1
...
echo "--- Running: vtctldclient ApplySchema --sql-file duo_schema_update.sql duo\n"
vtctldclient ApplySchema --sql-file duo_schema_update.sql duo || fail "Failed to apply schema for the duo keyspace"
...
echo "--- Running: vtctldclient ApplyVSchema --vschema-file vschema_duo_initial.json duo"
vtctldclient ApplyVSchema --vschema-file vschema_duo_initial.json duo || fail "Failed to apply vschema for the duo keyspace"
